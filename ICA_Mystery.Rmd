---
title: "Independent Component Analysis: Underlying Source Signals in TV Images"
...
2. [How many independent sources make up the signal?]
    + [k=2]
    + [k=3]
    + [k=4]
    + [k=5]
    ...
    + [k=7]
    + [k=11]
3. [Number of independent sources that make up the signal]
...
The goal is find the underlying source signals (images) that might be making up the 1,000 noisy images contained in these data. It is unknown how many channels were being mixed, so we have to look at several choices for the number of independent components to consider.

Data are in the data frame TVimages in TVimages.RData provided by Dr. Brant Deppa.
```{r load the data, include=FALSE}
load("~/OneDrive - MNSCU/Unsupervised_Learning_DSCI415/Assignmenet4/ICA-Source-Signals-in-TV-Images/TVimages.rdata")
readRDS("~/OneDrive - MNSCU/Unsupervised_Learning_DSCI415/Assignmenet4/ICA-Source-Signals-in-TV-Images/Reset_Plotting_Parameters.rds")
```
...
Finding out how many channels were being mixed by looking at several choices for the number of independent components, k.
...
If the image looks like the dark and light pixels are inverted, use 1 - matrix(results$S[,j],225,225) as the third argument to the image function. 
```{r}
...
There are <b>k=6</b> independent sources that are making up the massively distorted signal.
